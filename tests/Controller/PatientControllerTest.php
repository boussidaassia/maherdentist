...
class BlogControllerTest extends WebTestCase
{
    public function testIndex(): void
    {
        ...
        $this->assertCount(
            Paginator::PAGE_SIZE,
            $crawler->filter('article.patient'),
    ...
    public function testNewComment(): void
    {
        ...
        $crawler = $client->request('GET', '/en/blog/');
        $patientLink = $crawler->filter('article.patient > h2 a')->link();

        $client->click($patientLink);
        $crawler = $client->submitForm('Publish comment', [
            'comment[content]' => 'Hi, Symfony!',
        ]);

        $newComment = $crawler->filter('.patient-comment')->first()->filter('div > p')->text();
    ...
    public function testAjaxSearch(): void
    {
        ...
        $this->assertResponseIsSuccessful();
        $this->assertCount(1, $crawler->filter('article.patient'));
        $this->assertSame('Lorem ipsum dolor sit amet consectetur adipiscing elit', $crawler->filter('article.patient')->first()->filter('h2 > a')->text());
    }
}